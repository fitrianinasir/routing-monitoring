        ...
        supervisors: Object.keys(DATA[item]).map((secItm, i) => {
          return {
            ...
            salesmen: Object.keys(DATA[item][secItm]).map((thirdItm) => ({
              ...
              dates: Object.keys(DATA[item][secItm][thirdItm]).map((date) => ({
                salesman_name: thirdItm,
                date: date,
  ...
  return (
    <div>
      <div className="filter-table">
        {tableData.length > 0
          ? tableData.map((data) => (
              <table border="1" cellPadding="15" bordercolor="#D4D4D4">
                <tbody>
                  <tr>
                    <td colspan="10" className="manager-style">
                      {data.manager_name}
                    </td>
                  </tr>
                  <tr>
                    {data.supervisors.map((spv, i) => (
                      <td
                        colspan={spvCol[i].number_of_dates}
                        className="supervisor-style"
                      >
                        {spv.supervisor_name}
                      </td>
                    ))}
                  ...
                  <tr>
                    {data.supervisors.map((spv) =>
                      spv.salesmen.map((sales, i) => (
                        <td
                          colspan={sales.dates.length}
                          className="salesman-style"
                        >
                          {sales.salesman_name}
                        </td>
                      ))
                  ...
                  <tr>
                    {data.supervisors.map((spv) =>
                      spv.salesmen.map((sales) =>
                        sales.dates.map((date, i) => (
                          <td className="date-style">
                            {dateCoverter(date.date)}
                          </td>
                        ))
                      )
                    )}
